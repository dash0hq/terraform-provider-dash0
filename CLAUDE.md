...
- **Terraform and OpenTofu registries** consume `docs/index.md`, `docs/resources/*.md`, `docs/guides/*.md`. All three trees are regenerated by `make docs` (terraform-plugin-docs) and must not be edited by hand — edit the resource `Description` strings, the `templates/index.md.tmpl` file, or the corresponding `templates/guides/<slug>.md.tmpl` file instead. Any hand edit under `docs/` is wiped out on the next `make docs` run.
