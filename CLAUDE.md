...
The sync workflow only runs on release: `.github/workflows/release.yml` calls it with `dry-run: false` after `goreleaser` publishes a tag. There is no per-PR drift guard — a `replace-regex` rule that stops matching, or a new file that isn't declared in `files:` (nor listed under `coverage.ignore`), will only surface when the next release runs. When you add or rename a `docs/` file, update `transformations.yaml` in the same PR, and verify locally with a manual `workflow_dispatch` run of `Synchronize docs to dash0.com/docs` (dry-run checked) before merging anything that touches the docs tree or the transformations file.
