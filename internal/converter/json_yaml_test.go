...
// The document reported on #170, end to end: a yamlencode state value against a
// response whose permissions the platform rewrote and extended.
//
// Every line that differs from the reference is a real change except the quoting
// on the last two permission entries. The API returned them in a different order
// than state, and sequences align by position, so those entries match the wrong
// sibling and lose the spelling they would otherwise inherit.
func TestConvertAPIResponseToYAMLRendersTheReportedDocument(t *testing.T) {
	reference := `"kind": "Dash0View"
"metadata":
  "name": "request-logs"
"spec":
  "display":
    "description": "Visualization of load balancer logs."
    "name": "Request Logs"
  "filter":
  - "key": "service.name"
    "operator": "is"
...
`
	...
	apiResponse := `{"kind":"Dash0View","metadata":{"annotations":{},"labels":{"dash0.com/dataset":"default",` +
		`"dash0.com/id":"00000000-0000-0000-0000-000000000001",` +
		`"dash0.com/origin":"tf_00000000-0000-0000-0000-000000000002",` +
		`"dash0.com/source":"terraform","dash0.com/version":"1"},"name":"request-logs"},` +
		`"spec":{"display":{"description":"Visualization of load balancer logs.","name":"Request Logs"},` +
		`"filter":[{"key":"service.name","operator":"is","value":"checkout-service"}],` +
		`"permissions":[{"actions":["views:read","views:write","views:delete"],` +
		`"userId":"00000000-0000-0000-0000-000000000003"},` +
		`{"actions":["views:read","views:delete"],"role":"admin"},` +
		`{"actions":["views:read"],"role":"basic_member"}],` +
		`"table":{"columns":[{"colSize":"min-content","key":"otel.log.time","label":"Time"},{"key":"url.full"}],` +
		`"sort":[{"direction":"descending","key":"otel.log.time"}]},"type":"logs"}}`
