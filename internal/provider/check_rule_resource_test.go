...
// MockDash0Client is a mock implementation of dash0ClientInterface for testing
type MockDash0ClientCheckRule struct {
	mock.Mock
}

func (m *MockDash0ClientCheckRule) CreateDashboard(ctx context.Context, dashboard dashboardResourceModel) error {
	args := m.Called(ctx, dashboard)
	return args.Error(0)
}

func (m *MockDash0ClientCheckRule) GetDashboard(ctx context.Context, dataset string, origin string) (*dashboardResourceModel, error) {
	args := m.Called(ctx, dataset, origin)
	return args.Get(0).(*dashboardResourceModel), args.Error(1)
}

func (m *MockDash0ClientCheckRule) UpdateDashboard(ctx context.Context, dashboard dashboardResourceModel) error {
	args := m.Called(ctx, dashboard)
	return args.Error(0)
}

func (m *MockDash0ClientCheckRule) DeleteDashboard(ctx context.Context, origin string, dataset string) error {
	args := m.Called(ctx, origin, dataset)
	return args.Error(0)
}

func (m *MockDash0ClientCheckRule) CreateCheckRule(ctx context.Context, checkRule checkRuleResourceModel) error {
	args := m.Called(ctx, checkRule)
	return args.Error(0)
}

func (m *MockDash0ClientCheckRule) GetCheckRule(ctx context.Context, dataset string, origin string) (*checkRuleResourceModel, error) {
	args := m.Called(ctx, dataset, origin)
	return args.Get(0).(*checkRuleResourceModel), args.Error(1)
}

func (m *MockDash0ClientCheckRule) UpdateCheckRule(ctx context.Context, checkRule checkRuleResourceModel) error {
	args := m.Called(ctx, checkRule)
	return args.Error(0)
}

func (m *MockDash0ClientCheckRule) DeleteCheckRule(ctx context.Context, origin string, dataset string) error {
	args := m.Called(ctx, origin, dataset)
	return args.Error(0)
}
...
func TestCheckRuleResource_Configure(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			providerData: &MockDash0ClientCheckRule{},
			expectError:  false,
		},
		{
...
func TestCheckRuleResource_Create_InvalidYAML(t *testing.T) {
	mockClient := &MockDash0ClientCheckRule{}
